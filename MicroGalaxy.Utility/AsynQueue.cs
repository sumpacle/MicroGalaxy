...
namespace MicroGalaxy.Utility
{
    ...
    public static class BackgroundCacheTaskQueue
    {
        public static TimeSpan ExpirationTime { get; set; } = new TimeSpan(0, 20, 0);
        static Mutex mutex = new Mutex(true, "mutex1");

        private static ConcurrentDictionary<string, TaskResult> cache = new ConcurrentDictionary<string, TaskResult>();
        public static T Run<T>(string key, Func<string,T> task,string empid)
        {
            mutex.WaitOne();
            if (cache.ContainsKey(key))
            {
                ...
                else if (result != null && result.ExpirationTime <DateTime.Now)
                {
            ...
            var actionresult = task.Invoke(empid);
            cache.TryAdd(key, new TaskResult() { Result = actionresult, ExpirationTime = DateTime.Now.AddMilliseconds(ExpirationTime.TotalMilliseconds) });
            mutex.ReleaseMutex();
